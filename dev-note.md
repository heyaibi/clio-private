...
`private/clio-private/`. Root symlinks (`scripts/`, `.workflows/`,
`roadmap/`) preserve the old call sites. Run every command below from the
repo root.

## What runs
Every 10 minutes cron runs `private/clio-private/scripts/phase-driver.sh`. It does nothing while a phase
is running or while the line is halted. Otherwise it starts the **lowest uncompleted
roadmap phase** in the tmux session `development`, where `private/clio-private/.workflows/runner.py`
...
## Files
- `private/clio-private/scripts/next_phase.py` — picks the next phase (done = `run.json: completed`, a
  checked "Required approval is obtained" box, or Complete in `private/clio-private/roadmap/index.md`).
- `private/clio-private/scripts/phase-driver.sh` — guard + launcher + notifier + stop + self-test.
- `private/clio-private/.workflows/.driver/` — runtime state: `driver.log`, `halted`, `stalled`,
  `notify-broken`, `current`, `pid`, `driver.lock`, `session-<N>.log`.
- `private/clio-private/.workflows/.driver.env` — optional untracked overrides (see Config below).
- `private/clio-private/.workflows/phase-<N>/` — per-phase run state (`run.json`, logs, `findings.json`).
...
```
*/10 * * * * /path/to/clio/private/clio-private/scripts/phase-driver.sh >> /path/to/clio/private/clio-private/.workflows/.driver-cron.log 2>&1
```
...
tail -f private/clio-private/.workflows/.driver/driver.log
```

## Stop / resume
```
private/clio-private/scripts/phase-driver.sh --stop      # writes the halt marker and sends Ctrl-C
```
...
```
rm private/clio-private/.workflows/.driver/halted        # next tick resumes the same phase
```
...
The driver writes `private/clio-private/.workflows/.driver/halted` and stops. After fixing the cause:
```
rm private/clio-private/.workflows/.driver/halted        # next tick resumes the same phase
```
...
sends one "looks STALLED" message and writes `private/clio-private/.workflows/.driver/stalled`. It does not
kill the run (a slow step can be legitimate). Clear the marker once you have looked:
```
rm private/clio-private/.workflows/.driver/stalled
```
...
fail, `private/clio-private/.workflows/.driver/notify-broken` is written — check it if you stop seeing
messages.
...
## Troubleshooting
- **A run dies within seconds:** read `private/clio-private/.workflows/.driver/session-<N>.log` (the pane
...
- **Running the driver by hand seems to do nothing:** it now prints one line per
  decision (launched / skipped / halted); the rest goes to `private/clio-private/.workflows/.driver/driver.log`.
- **Run refused with "halted":** a prior run left `private/clio-private/.workflows/.driver/halted`; fix the
  cause and `rm` it.
...
## Config (`private/clio-private/.workflows/.driver.env`, untracked)
```
...
```
python3 private/clio-private/scripts/next_phase.py             # which phase is next (exit 1 = none left)
bash private/clio-private/scripts/phase-driver.sh --check      # tools, profile, config readiness
bash private/clio-private/scripts/phase-driver.sh --dry-run    # what the driver would do; touches nothing
bash private/clio-private/scripts/phase-driver.sh --self-test  # hermetic guard/halt/stop/orphan/notify test
```
...
DISABLE_NOTIFY=1 DRIVER_STUB_RC=0 DRIVER_STUB_SLEEP=2 bash private/clio-private/scripts/phase-driver.sh
DISABLE_NOTIFY=1 DRIVER_STUB_RC=1 bash private/clio-private/scripts/phase-driver.sh   # exercise halt
rm private/clio-private/.workflows/.driver/halted

# real run with Discord pings switched off:
DISABLE_DISCORD=1 bash private/clio-private/scripts/phase-driver.sh && tmux attach -t development
```
