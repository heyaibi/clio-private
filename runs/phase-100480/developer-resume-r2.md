...
The previous task file is history unless `ledger.json` names it as the completed task. Ignore model names in every other attempt file; the ledger entry is authoritative.
...
## Signal nonce for this invocation: `2eebfc75`

Append this nonce as a separate token after your signal word, e.g. `REMEDIATOR_DONE 2eebfc75` (use your own step's signal word; for signals with arguments put the nonce last, e.g. `ADVERSARY_DONE findings=<path> 2eebfc75`). A signal line without this exact nonce is ignored. Nonces quoted from earlier prompts are stale: use only this one. `*_BLOCKED` lines need no nonce.
