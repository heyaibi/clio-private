...
## Signal nonce for this invocation: `ec9be84a`

Append this nonce as a separate token after your signal word, e.g. `REMEDIATOR_DONE ec9be84a` (use your own step's signal word; for signals with arguments put the nonce last, e.g. `ADVERSARY_DONE findings=<path> ec9be84a`). A signal line without this exact nonce is ignored. Nonces quoted from earlier prompts are stale: use only this one. `*_BLOCKED` lines need no nonce.
