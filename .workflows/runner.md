...
A list of identifiers means round-robin: each invocation of that step uses the next entry, wrapping around. Counters persist across runs in `.workflows/.harness-rotation-<pipeline>-<hash>.json` (one file per pipeline file, keyed by pipeline path), so consecutive runs keep alternating. Rotation slots are consumed only after a successful invoke, so a crash between render and invoke retries the same harness. `--fresh` does not reset rotation.
