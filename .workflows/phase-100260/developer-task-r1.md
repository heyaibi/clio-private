...
## Signal nonce for this invocation: `0d0efcea`

Append this nonce as a separate token after your signal word, e.g. `REMEDIATOR_DONE 0d0efcea` (use your own step's signal word; for signals with arguments put the nonce last, e.g. `ADVERSARY_DONE findings=<path> 0d0efcea`). A signal line without this exact nonce is ignored. Nonces quoted from earlier prompts are stale: use only this one. `*_BLOCKED` lines need no nonce.
