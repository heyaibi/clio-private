...
Phase 100640 is complete (PASS): the `Reranker` trait now returns ordered `(index, score)` pairs carrying the provider's normalized relevance in `[0, 1]` (identity on `[0, 1]` with a defensive clamp; TEI pins `raw_scores: false`), `scores.reranker` is populated on reranked hits and `null` on passthrough/error/omitted documents and for index-only response entries (the pre-score `results[]`/bare-array compatibility semantics are preserved), and fail-open, full permutation, and fail-closed index checks are unchanged. The Phase 100300 trait amendment (return shape only) is recorded in the phase's completion evidence; the plan-time owner grant has no separate artifact and is pending owner confirmation at the phase's §12 sign-off.
...
| 100640 | Rerank relevance capture and per-provider normalization | ~3–4 days | [phase-100640-rerank-relevance-normalization.md](phase-100640-rerank-relevance-normalization.md) | Phase 100620; Phase 100421 TEI contract — Complete — PASS |
