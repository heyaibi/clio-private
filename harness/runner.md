...
A list of identifiers means round-robin across runs, not across the rounds of one run. Repeating an identifier gives it proportional weight; interleave repeated entries with other identifiers when the desired ratio permits it.

## The per-run harness plan

Before the job starts, the runner decides the whole map: which of a stage's declared identifiers every stage will use for this run. It saves that map to `<run_dir>/harnesses.json` — one file per phase run dir — and the run then uses it unchanged, so every round of a loop step, every retry, every rendered prompt, and every `Attribution` row name the same model. Two ways to produce it, one outcome:

- **Default:** the runner decides alone, starting from the durable rotation counter.
- **`--customize-harness`:** the operator picks each stage's slot from the options that stage declares, then confirms the whole map. It offers only the stage's own identifiers (stage files own those, with their display names and effort checks) and only the ones whose CLI is installed, marked per option. Answer with the option number, the identifier, or the display name; Enter keeps the current choice; `q` aborts. A stage with a single installed option is not asked about. The map is printed and confirmed before anything starts, and declining (or `q`, or Ctrl-C) starts nothing and exits 2 with `{"state": "aborted", ...}`. It needs a terminal: a piped or redirected run is refused rather than left waiting for a keystroke. A stage the ledger already completed is shown and kept — its harness is part of the completion record, so re-planning cannot rewrite it.

Both write the same file, so a `--customize-harness` run and a default run are indistinguishable downstream. The plan is decided before the preflight sync and the phase claim, so a plan the run cannot honour (nothing installed, a declined plan) stops it having claimed nothing.

Rotation lives in `private/clio-private/runs/.harness-rotation-<pipeline>-<hash>.json`, one file per pipeline file, keyed by pipeline path. The plan starts from that counter and advances it once per run per step, when the step is accepted, so consecutive runs alternate. A missing signal, blocked result, missing required artifact, or interrupted attempt advances nothing, and the saved plan is reused on resume either way. A resumed run keeps its saved plan: a step's entry survives while the stage still declares it and its CLI is still installed, and is re-decided only when a pipeline edit dropped the identifier, the binary went away, or a pipeline edit added a step the plan does not know. A plan that is present but unreadable fails closed (exit 2, "delete it to replan this run, or use --fresh") rather than silently switching a stage's model mid-run. A write failure fails the run, because an unsaved plan must not start the job.

Availability is still binary presence only, and the plan records the same skips the invoke-time walk reports: each entry names the identifier and the missing binary, the step banner and the run event repeat it, and the accepted invocation advances the counter past the skips so a later run does not retry a known-missing head. When every entry for a stage is missing its binary the run halts before the job starts as a config error (exit 2) whose JSON names the step and every missing binary; nothing is invoked, no ledger entry or rotation advance is written, and `resume.json` stays on the step so installing a harness and resuming retries it. `--dry-run` prints the same map (and, with `--customize-harness`, prompts for it) but saves nothing.

See "The per-run harness plan" above for when availability is checked and what a skip records.
...
Current step selections (kept in sync with the operator-owned `preference.md`, which is the source of truth for these lists; edit the stage frontmatter to match it, never the reverse): developer rotates `opencode:go/deepseek-v4.1-flash@max`, `opencode:together/glm-5.3-flash@high`, `opencode:go/space-bunny-free@max`, and `opencode:openrouter/deepseek-v4.1-flash@max`; adversary rotates `opencode:go/space-bunny-free@max`, `opencode:go/space-bunny-free@max`, `opencode:openrouter/deepseek-v4.1-flash@max`, and `opencode:go/deepseek-v4.1-flash@max`; remediator rotates `opencode:together/glm-5.3-flash@high`, `cmd:deepseek/deepseek-v4-flash@max`, and `opencode:go/space-bunny-free@max`; approver rotates `opencode:go/space-bunny-free@max`, `opencode:go/space-bunny-free@max`, and `cmd:deepseek/deepseek-v4.1-flash@max`; finalize rotates `opencode:together/glm-5.3-flash@high`, `cmd:deepseek/deepseek-v4-flash@max`, and `opencode:go/space-bunny-free@max`. No stage selects `agy`; the runner still knows the `agy` CLI identifier, so an old ledger or task file naming it stays readable.
...
Harness-plan check (hermetic, no inference spend, no repo state): `python3 private/clio-private/harness/check-harness-plan.py` puts stub harness binaries in a temp PATH with HOME at the same temp root, then drives the real `runner.main()` (only the network-bound sync and the coordination-bound phase claim are stubbed). It asserts decided-before-start (the plan names every stage, the stub sees `harnesses.json` already on disk, and the ledger agrees with it), one-harness-per-stage (a three-round loop step uses the planned harness every round and consumes one rotation slot for the whole run), operator-choice (`--customize-harness` through a real pty: the typed pick is shown, confirmed, saved before the run, and used), declined-plan (answering no runs nothing, saves no plan, writes no ledger/resume/run state, and reports `state: aborted`), and resume-keeps-plan (a missing-signal failure then a resume reuses the same harness and does not rewrite the plan).
